#!/usr/bin/env node
/**
 * Nebenkosten Studio – Lizenzschlüssel-Werkzeug (Verkäuferseite).
 *
 * Erzeugt ECDSA-P-256-Schlüsselpaare und signiert Lizenzschlüssel im Format
 *   NKS1.<base64url(payload)>.<base64url(signatur)>
 *
 * Befehle:
 ...
 *   node tools/keygen/keygen.mjs verify --key NKS1....
 *
...
export async function signLicense(privateKey, { email, plan = "pro", iat }) {
  ...
  const data = new TextEncoder().encode(`NKS1.${payloadB64}`);
  const sig = await crypto.subtle.sign({ name: "ECDSA", hash: "SHA-256" }, privateKey, data);
  return `NKS1.${payloadB64}.${b64url(new Uint8Array(sig))}`;
}
...
async function cmdVerify(args) {
  ...
  if (parts.length !== 3 || parts[0] !== "NKS1") {
    console.error("Ungültiges Format (erwartet: NKS1.<payload>.<signatur>).");
    process.exit(1);
  }
  const data = new TextEncoder().encode(`NKS1.${parts[1]}`);
